var MorePage = {
  render: function() {
    ...
    var sections = [
      ...
      },
      {
        ...
        items: [
          ...
          { icon:'📅', label:'Daily Report',         desc:'Print today\'s daily report',       bg:'var(--gb)',   action:"Reports.switchToDaily();Router.go('reports')"},
          { icon:'📅', label:'Expense Allocations', desc:'Set up recurring cost allocations', bg:'rgba(245,158,11,.12)', action:"Allocations.render()", badge:'NEW', badgeColor:'var(--wa)' },
        ]
      },
      {
        label: 'System',
    ...
    var html = '<div class="page-header">'
      + '<div><div class="page-title">More</div><div class="page-sub">Features & settings</div></div>'
      + '</div>';
    ...
    sections.forEach(function(section) {
      ...
      section.items.forEach(function(item) {
        var badgeHtml = item.badge
          ? '<span style="background:'+(item.badgeColor||'var(--g)')+';color:#07080D;font-size:9px;font-weight:800;padding:2px 8px;border-radius:99px;flex-shrink:0">'+item.badge+'</span>'
          : '';
        html += '<div class="more-item" onclick="'+item.action+'">'
          + '<div class="more-icon" style="background:'+item.bg+'">'+item.icon+'</div>'
          + '<div class="more-text">'
          + '<div class="more-name"'+(item.danger?' style="color:var(--er)"':'')+'>'+item.label+'</div>'
