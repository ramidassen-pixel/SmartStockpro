...
var Auth = {
  ...
  signup: function() {
    ...
    Auth._sendEmail({
      ...
      name: name,
      bizName: bizName,
    }, verifyCode).then(function(result) {
      if (result.sent) {
        Auth._showEmailVerification(user, null); // hide code on screen
        Toast.show('Verification email sent to ' + email, 'ok');
      } else {
        Auth._showEmailVerification(user, result.code); // show code as fallback
      }
    });
  },
  ...
  _continueJoin: function(biz, bizName, name, email, phone, pw) {
    ...
    Auth._sendEmail({
      ...
      if (result.sent) {
        Auth._showEmailVerification(user, null);
        Toast.show('Verification email sent to ' + email, 'ok');
      } else {
        Auth._showEmailVerification(user, result.code);
      }
    });
  },
  ...
  sendResetCode: function() {
    ...
    Auth._sendEmail({
      ...
      var codeLine = result.sent
        ? '<div style="background:var(--okb);border:1px solid var(--okbd);border-radius:var(--r10);padding:12px;margin-bottom:14px;font-size:12px;color:var(--ok)">✅ Reset code sent to your email.</div>'
        : '<div style="background:var(--wab);border:1px solid var(--wabd);border-radius:var(--r10);padding:12px;margin-bottom:14px;font-size:12px;color:var(--wa)">📧 Your reset code is: <strong style="font-size:16px;letter-spacing:4px">' + (result.code||code) + '</strong></div>';
  ...
  approveUser: function(userId, role) {
    ...
    // Send approval email
    var approvedUser = users[idx];
    var bizArr = DB.get('businesses') || [];
    var biz = bizArr.find(function(b){ return b.id === (approvedUser.businessIds||[])[0]; });
    Auth._sendEmail({
      to: approvedUser.email,
      type: 'approved',
      name: approvedUser.name,
      bizName: biz ? biz.name : 'SmartStock Pro',
    }, null);
  ...
  _sendEmail: function(payload, fallbackCode) {
    ...
    .then(function(data) {
      if (data.success) {
        ...
        console.warn('Email API issue:', data.error);
