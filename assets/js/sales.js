var Sales = {
  ...
  shareWhatsApp: function(saleId) {
    ...
    // Build clean text receipt
    var lines = [];
    lines.push('🧾 *INVOICE — ' + bizName + '*');
    lines.push('─────────────────────────');
    lines.push('📋 Invoice: *' + s.id + '*');
    lines.push('📅 Date: ' + Utils.date(s.date));
    lines.push('👤 Customer: ' + Utils.esc(s.customer||'Walk-in Customer'));
    lines.push('');
    lines.push('*ITEMS PURCHASED:*');
    (s.items||[]).forEach(function(item){
      var lineTotal = (parseFloat(item.price)||0)*(parseInt(item.qty)||1);
      lines.push('• ' + item.name + ' × ' + item.qty + ' = ' + Utils.cur(lineTotal,cur));
    });
    lines.push('─────────────────────────');
    if ((s.discount||0)>0) {
      lines.push('Discount: ' + s.discount + '%');
    }
    lines.push('💰 *TOTAL: ' + Utils.cur(s.total,cur) + '*');
    lines.push('✅ Status: ' + (s.status||'Paid'));
    if (parseFloat(s.balance)>0) {
      lines.push('⚠️ Balance Due: ' + Utils.cur(s.balance,cur));
    }
    ...
    // Try native Web Share API first (Android Chrome supports it)
    if (navigator.share) {
      navigator.share({
        title: 'Invoice ' + s.id + ' — ' + bizName,
        text:  msg,
      }).catch(function(e) {
  ...
  shareWhatsAppCustomer: function(saleId) {
    ...
    lines.push('─────────────────────────');
    lines.push('💰 *TOTAL: ' + Utils.cur(s.total,cur) + '*');
    lines.push('✅ ' + (s.status||'Paid'));
    if (parseFloat(s.balance)>0) lines.push('⚠️ Balance: ' + Utils.cur(s.balance,cur));
    lines.push('');
